...
Predation, generalised into the organising principle of a system, is **malignant**.
Not as an insult — as a diagnosis. A malignant growth is not "cell division being
...
the collaborative force is older, deeper, and still here.

## 2. The collaborative ground is the older layer
...
it is late (phagocytosis ~1.2 Ga; `S017`), some that predation is primordial ("No
Garden of Eden," de Nooijer et al. 2009; `S018`). That second argument rests on
...
The cooperative force is not an ideal we preach. It is a mechanism the record keeps
showing:
...
organisms into societies — succeeded by *limiting the conflict at the lower level*
so a larger cooperating whole could hold. `[E:C001, E:C002-source S002]`

## 4. The new stage

For most of life's history this ran blind. No lineage could see the consequences of
its own strategy for the larger whole, let alone change them. Evolution had no
foresight.

Then it produced an agent that can **recognise** the destructive generalisation of
predation and is able to **change** it. That recognition is not a judgement of
evolution from outside — evolution has no plan to betray. It is evolution reaching a
...
substrate they stand on — when the collaborative force, which was always the older
ground, is chosen again, on purpose, at the scale where it now matters most.

## 5. What would refute all of this

- If systems organised around unleashed extraction — no cooperation, no effective
  dissent, no conflict control — reliably flourish over long horizons instead of
  tipping, the malignancy thesis fails. `[falsifier: C011]`
- If cooperation and conflict-control mechanisms do **not** predict the stability of
  major transitions or multi-agent systems, the collaborative-ground claim fails.
  `[falsifier: C001]`
- If the claimed capacity for insight and correction is illusory — if the "moral
  agent" only disguises its own predation — the new-stage claim fails. `[falsifier:
  C010]`

## 6. Boundaries we do not cross
...
testing. The correction to predatory power must never become another predatory
power that suppresses critics in the name of "cheater control." A single predator
in a feedback-bound ecosystem is not the cancer; the decoupled, feedback-escaping
pattern is. We name the malignant pattern boldly and we refuse to smear the living
animal, because the second would make the first easy to dismiss.
